...
print -n "Creating required directory tree... "
...
if type -f make &> /dev/null; then
    # Make mongo-hacker
    print -n "Making mongo-hacker config... "
...
if type -f perl &> /dev/null; then
    ...
    print -n "Installing diff-so-fancy... "
...
# Link nodenv plugins
print -n "Linking nodenv plugins... "
...
print -n "Installing cron job for periodic updates... "
...
if cat <(fgrep -i -v "$CRON_TASK" <(crontab -l)) <(echo "$CRON_SCHEDULE") | crontab - &> /dev/null; then
    print "done"
else
